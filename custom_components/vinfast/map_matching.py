...
def trim_route_to_projections(offset_route, coords):
    """Cắt xén đầu/đuôi lộ trình AI sao cho dừng lại đúng mép đường, không đâm vào nhà"""
...
def light_cleanup(coords, min_dist=2.0):
    ...
    if haversine_distance(cleaned[-1][0], cleaned[-1][1], coords[-1][0], coords[-1][1]) > 0.5:
...
async def async_process_route(hass, raw_coords, mapbox_token, stadia_token):
    ...
    # ==============================================================
    # CÁCH LY VÙNG NHIỄU (ZONE SPLITTING) 
    # Lọc bỏ khu vực bãi đỗ xe/hẻm nhỏ để AI không bị nhầm lẫn
    # ==============================================================
    start_road_idx = 0
    end_road_idx = len(raw_coords) - 1
    
    for i in range(len(raw_coords)):
        spd = raw_coords[i][2] if len(raw_coords[i]) > 2 else 0
        if spd >= 12.0:
            start_road_idx = max(0, i - 2) 
            break
            
    for i in range(len(raw_coords)-1, -1, -1):
        spd = raw_coords[i][2] if len(raw_coords[i]) > 2 else 0
        if spd >= 12.0:
            end_road_idx = min(len(raw_coords) - 1, i + 2) 
            break
            
    if start_road_idx >= end_road_idx - 5:
        start_road_idx, end_road_idx = 0, len(raw_coords) - 1
        
    road_coords = raw_coords[start_road_idx : end_road_idx + 1]
    _LOGGER.warning(f"[2] TRÍCH XUẤT ĐƯỜNG TRƯỜNG: Bỏ qua vùng bãi đỗ, gửi {len(road_coords)} điểm lên AI.")

    cleaned_raw = light_cleanup(road_coords, min_dist=2.0)
    ...
    final_offset_route = trim_route_to_projections(offset_route, raw_coords)
